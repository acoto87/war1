...
* [x] When catapults are close enough to buildings and attack them, may destroy themselves because of the splash damage.
* [x] When editing walls, if trap an enemy unit and order a friendly unit to attack it removing in the process a piece of the wall, the friendly unit can't get to the enemy unit. Probably because I'm not updating the pathfinder information when deleting pieces of walls.
* [x] When "Ides of march" cheat is used behind the panel the actions and sounds continues. That shouldn't be.
...
* [x] Frame rate gets unacceptable in debug mode (~5 fps after a while and a few units created)
* [x] MENU F10 tooltip overlaps if there a building training a soldier or worker saying "TRAINING ..."
* [x] Cancel button tooltip is not showing
* [x] FPS and dt information in the window title render every frame, hard to see actual framerate. Title should be updated every frame, but fps, dt and wait time only each second. Actual time and frame count should be updated every frame, tho.
* [x] Magnifying glass to select units is not showing / cursor is not chaging
...
* [x] I can hear the sounds of the enemy spawning and doing stuff
* [x] If several corpses are in the same place, there is a sort-fighting to determine which corpse is rendered on top of the others, and it can cause a flickering effect.
* [x] Sometimes with corpses I'm getting: WARNING: [war_actions.c:(1087)]: Unit type 52 does not have an action of type 0 because somebody set the action IDLE to a corpse.
* [x] When I create a unit through `createUnit` function, it automatically set the IDLE state, allocating the state, but sometimes (e.g., when a unit dies and a corpse is created `war_state_machine_death.c`) is immediately set to the DEATH state, so the IDLE state is never used and the memory allocated for it is wasted. Maybe I can add a parameter to `createUnit` to set the initial state, or maybe just create the unit without setting any state and let the caller set the initial state.
...
* [ ] When I create a custom game, it doesn't have any building or units, so the player can't do anything, and immediately the Your are victorious message appears. Then, upon clicking the Continue button, I'm getting this assertion fail: Assertion failed: index < length, file C:\Users\Alejandro\Projects\warcraft\war1-c\src\war_campaigns.c, line 371
...
* [x] Make commands/cheats system.
    ...
    * [x] Custom cheats
        ...
        * [x] Change edition of trees to cheat
        * [x] Change edition of ruins to cheat
        * [x] Change edition of walls to cheat
...
* [x] Write a detailed description of the actions system, maybe as comments in the `war_actions.c` file?
* [x] Manage components with a dictionary and not each entity having all the components.
* [x] Create a mechanism for all debug information currently enabled by macros to be enabled/disabled at runtime, and not at compile time. This will allow to enable/disable debug information without recompiling the game.
...
* [ ] Rethink the state machine system:
    * [ ] Keep state machine but implement a push-down (stack based) state machine, that allows to push and pop states, so for example, when a unit is attacking and is attacked, it can push the attack state of the attacker, and then pop it to return to the previous attack state.
    * [ ] Remove state machine and implement behavior trees or something like that. This will allow more complex behaviors.
...
> * [ ] Rethink the pathfinding system
    * [x] Add a data structure (e.g., a grid) to find nearby units
    ...
    * [ ] Need to rethink the actual walking of the units, now they follow the paths segment by segment, but maybe it would be better to follow the path as a whole
