...
* [x] When the AI is harvesting gold or lumber, it's reflected on the player numbers in the top bar.
...
* [ ] Check all `*ListFree(...)` invocations to determine if the actual list pointer need to be freed. Do the same with all other collections. Can I introduce a parameter in those `*Free(...)` functions that say something like: `*Free(..., bool freeList)` or something
* [ ] Handle when there is a building being constructed that for some reason has no worker building it.
* [ ] When there is several units, and select 4 to move, fps drops to 20 or so.
* [ ] When things get cluttered around the TownHall, the current implementation of `isAccessibleUnit` can fail because there can be no path between the closest tiles of each units, but there is a path between other tiles of the units.
...
* [ ] FoW to be player-based, and not map-based
    ...
    * [x] Review that the correct player is retrieved for querying fow in each scenario.
        ...
        indicating the player that the UI will render information of. An observer is an special player that doesn't have a corresponding
        WarPlayerInfo in the map.
        When there is NOT and observer, the uiPlayer field on WarMap must be always 0 (the human player).
        There can be up to 5 players in a map like:
        players[0] = human or computer
        players[1] = computer
        players[2] = computer or none
        players[3] = computer or none
        players[4] = neutral
        Player 5 is reserved for neutral units.
...
* [x] Create AI sub-systems that will handle each aspect of the game, resources, builds, trainings, attacks, etc.
* [x] Implement Mean-End-Analysis to generate a plan.
    * [ ] Include upgrades in the generation of the plan.
    * [ ] Implement plan execution.
* [ ] Implement attacking system
* [ ] Implement expansion system
    * [ ] Handle when the mines depletes or when there is no more trees in the resource system.
* [ ] Implement better building place selection, maybe around the TownHall with some rules about distances between them?
