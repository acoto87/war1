...
void updateAttackState(WarContext* context, WarEntity* entity, WarState* state)
{
    ...
    if (!targetEntity)
    {
        ...
        if(!unitTileInRange(entity, targetTile, 1))
        {
    ...
    if(isWall(targetEntity) && !unitTileInRange(entity, targetTile, stats.range))
    {
    ...
    {
        sendToWaitState(context, entity, 1.0f, state);
        return;
    }
    ...
    if (action->lastActionStep == WAR_ACTION_STEP_ATTACK)
    {
        ...
        if (isUnit(targetEntity))
        {
            ...
            if (isDeadUnit(targetEntity) || isCollapsedUnit(targetEntity))
            {
                sendToIdleState(context, entity, true);
            }
            else
