...
// On Windows, when Tracy is enabled and we are not building the DLL itself,
// default to __declspec(dllimport) so the pre-built Tracy.dll is used correctly.
#if defined _WIN32
#  if defined TRACY_EXPORTS
#    define TRACY_API __declspec(dllexport)
#  elif defined TRACY_ENABLE && !defined TRACY_STATIC_LINK
